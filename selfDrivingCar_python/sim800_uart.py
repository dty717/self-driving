...
def sim_checkGPRSContext():
    ...
    if res == b'\r\n':
        res = uart.readline()
        print(res)
    if res.startswith(b'+SAPBR: 1,3,'):
       res = uart.readline()
       print(res)
       return False
    elif res.startswith(b'+SAPBR: 1,1,') and res[13:20] != b'0.0.0.0':
        res = uart.readline()
        print(res)
        if res == b'\r\n':
            res = uart.readline()
...
def sim_setNTPServer(ntpServer,timeZone):
    simpleHandle(uart.read())
